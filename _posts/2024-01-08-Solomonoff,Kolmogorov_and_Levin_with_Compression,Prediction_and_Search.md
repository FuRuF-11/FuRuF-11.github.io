...
从通用先验出发，我们可以使用贝叶斯公式来描述整个SI。

设字符串为x，下一个需要预测的字符为a，产生x的程序为$\mu()$，我们有：

$$
M(xa|x)=\frac{M(xa)}{M(x)}
$$

请注意此处我们使用的是通用先验概率来定义的。
...
不过这个算法开销太大（NP-hard），我们依旧难以沿着这种思路实现一个优秀的归纳推理算法。

## Compression=Intelligence

SI理论最伟大的地方在于，我们可以从SI得出"**compression=intelligence**"这个极为有力的结论。

我们首先需要理清其SI理论与现代人工智能的关系（或者说与深度学习领域的关系）。

首先，Ilya提出，ANN实际上可以看作一台执行多步程序的多路并行的计算机（这个想法与莱文通用搜索不谋而合）。例如，一个50层的网络运行的过程就是同时也运行n个可以执行50步的程序，然后将这些程序的输出加权。优化网络的过程就是搜索程序的过程。

随后，我们可以看到，当我们使用神经网路（如GPT）进行完形填空式的自监督学习的时候，我们实际上是在解决所罗门所预定义的问题，即对于一个字符串，预测其下一个是什么。结合上面所说的ANN是对SI的近似的结论。我们可以认为现代的各种LLM，都是在执行一种近似于SI的归纳推理。而这种自监督学习的方法，确实使得GPT4这些模型产生了一部分智能。
...
因此，如果使用SI理论来总结和说明LLM的智能涌现现象的话，“compression=intelligence”这个公式足以。
